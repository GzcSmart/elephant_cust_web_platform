      ...
      function multipleBox(target, params) {
        ...
        for (var item in params.data) {
          if (params.data.hasOwnProperty(item)) {
            ...
            if (params.idVal) {
              html.push('<option ' + params.metaTag + '="' + oItem[params.id] + '" value="' + oItem[params.id] + '"' + (params.selected && params.selected[0] == oItem[params.name] ? hasSelected : '') + '>' + oItem[params.name] + '</option>');
            } else {
              html.push('<option ' + params.metaTag + '="' + oItem[params.id] + '" value="' + oItem[params.name] + '"' + (params.selected && params.selected[0] == oItem[params.name] ? hasSelected : '') + '>' + oItem[params.name] + '</option>');
            }
          }
        }
        html = html.join('');
                    ...
                    if (oItem[params.id] == extra && oItem[params.children]) {
                      ...
                      for (var sItem in oItem) {
                        if (oItem.hasOwnProperty(sItem)) {
                          if (params.idVal) {
                            html.push('<option ' + params.metaTag + '="' + oItem[sItem][params.id] + '" value="' + oItem[sItem][params.id] + '"' + (params.selected && params.selected[1] == oItem[sItem][params.name] ? hasSelected : '') + '>' + oItem[sItem][params.name] + '</option>');
                          } else {
                            html.push('<option ' + params.metaTag + '="' + oItem[sItem][params.id] + '" value="' + oItem[sItem][params.name] + '"' + (params.selected && params.selected[1] == oItem[sItem][params.name] ? hasSelected : '') + '>' + oItem[sItem][params.name] + '</option>');
                          }
                        }
                      }
                      break;
